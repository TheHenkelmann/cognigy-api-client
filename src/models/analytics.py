...
class CallCounterMetric(BaseModel):
    ...
    billable_calls: Optional[float] = Field(
        ...
        alias="billableCalls",
        ge=0,
        description="Number of calls that are billable."
    )
...
class ChannelConversations(BaseModel):
    ...
    channel: str = Field(
        ...,
        min_length=1,
        description="The name/identifier of the communication channel."
    )
    conversations: float = Field(
        ...,
        ge=0,
        description="Number of conversations on this channel."
    )
...
class ConversationCounterMetric(BaseModel):
    ...
    conversations: Optional[float] = Field(
        None,
        ge=0,
        description="Total number of conversations for this day."
    )
    per_channel: Optional[List[ChannelConversations]] = Field(
        None,
        alias="perChannel",
        description="Breakdown of conversations by channel."
    )
